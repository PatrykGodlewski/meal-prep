...
export const IngredientReplacementsSelector = React.memo(
  function IngredientReplacementsSelector({
    ...
    const quantity = useWatch({ control, name: `ingredients.${index}.quantity` }) ?? 0;
    const unit = useWatch({ control, name: `ingredients.${index}.unit` }) ?? "g";
    ...
    return (
      <FormField
        ...
        render={({ field }) => {
          const selected = (field.value ?? []) as { ingredientId: string; ratio?: number }[];
          const displayText =
            selected.length > 0
              ? selected
                  .map((s) => {
                    const opt = replacementOptions.find((o) => o.value === s.ingredientId);
                    const ratio = s.ratio ?? opt?.defaultRatio ?? 1;
                    const calculated = Math.round((quantity as number) * ratio);
                    return opt
                      ? `${opt.label} (${calculated} ${unit})`
                      : null;
                  })
                  .filter(Boolean)
                  .join(", ")
          ...
          return (
            <div className="col-span-full">
              <FormItem>
                <FormLabel className="text-xs">{tMeal("overrideReplacements")}</FormLabel>
                <Popover
                  open={popoverOpen}
                ...
                >
                  <PopoverTrigger asChild>
                    <FormControl>
                      ...
                      >
                        {displayText ?? tMeal("overrideReplacementsPlaceholder")}
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          {replacementOptions.map((opt) => {
                            ...
                            const ratio =
                              entry?.ratio ?? opt.defaultRatio ?? 1;
                            return (
                              <CommandItem
                                key={opt.value}
                                value={opt.label}
